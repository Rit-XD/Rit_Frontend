...
export const LoginSteps:React.FC = () => {
    ...
    let current = 1
    if (state.step === 'password') current = 2

    return (
        <form className="animate-in flex-1 flex flex-col w-full justify-center gap-2 text-background" action={action}>

        <label className="text-md" htmlFor="username">
          E-mail
        </label>
        <input className="px-4 py-3 bg-inherit border border-slate-200 rounded-full focus:outline-orange-500 focus:border-0 focus:outline-none" name="username" placeholder="naam@voorbeeld.com" required />
        ...
        <SubmitButton type="submit" className="px-4 py-2 text-foreground mb-2 rounded-full bg-orange-500" pendingText="Signing In...">
          Log in
        </SubmitButton>
